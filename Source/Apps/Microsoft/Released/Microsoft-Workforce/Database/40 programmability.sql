...
CREATE PROCEDURE wf.sp_get_replication_counts
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE PROCEDURE wf.sp_get_pull_status
AS
BEGIN
    SET NOCOUNT ON;
	...
	FROM wf.[configuration] WHERE configuration_group = 'SolutionTemplate' AND configuration_subgroup = 'Notifier' AND [name] = 'DeploymentTimestamp';

    DECLARE @InitialStatusDone NVARCHAR(4);

	SELECT @InitialStatusDone = [value]
	FROM wf.[configuration] WHERE configuration_group = 'SolutionTemplate' AND configuration_subgroup = 'Notifier' AND [name] = 'InitialPullDone';

	IF EXISTS (SELECT * FROM wf.[configuration] WHERE configuration_group = 'SolutionTemplate' AND configuration_subgroup = 'Notifier' AND [name] = 'ASDeployment' AND [value] ='true')
	SET @ASDeployment = 1;
    ...
    -- AS Flow
    IF @ASDeployment=1 AND DATEDIFF(HOUR, @DeploymentTimestamp, Sysdatetime()) > 24 AND NOT EXISTS (SELECT * FROM wf.ssas_jobs WHERE [statusMessage] = 'Success')
		SET @StatusCode = 1;
	ELSE 			
	BEGIN
		IF (@InitialStatusDone = 'True' AND EXISTS (SELECT * FROM wf.ssas_jobs WHERE [statusMessage] = 'Success'))
		SET @StatusCode = 2; --Data pull complete
	END;
