...
BEGIN 
    SET @stmt = 'DROP TABLE wf.' + QuoteName(@p1);
    EXEC (@stmt);
    FETCH NEXT FROM @cr INTO @p1;
END;
CLOSE @cr;
...
SET @cr = CURSOR FAST_FORWARD FOR
              ...
              WHERE ROUTINE_TYPE='PROCEDURE' AND 
                    ( (ROUTINE_SCHEMA='wf' AND ROUTINE_NAME IN ('sp_get_replication_counts', 'sp_get_prior_content', 'sp_get_last_updatetime','sp_get_pull_status'))
                    );
OPEN @cr;
...
BEGIN
    EXEC ('CREATE SCHEMA wf AUTHORIZATION dbo'); -- Avoid batch error
END;
