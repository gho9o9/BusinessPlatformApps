...
-- =============================================
IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='wf' AND TABLE_NAME='ssas_jobs' AND TABLE_TYPE='BASE TABLE')
...
INSERT wf.[configuration] (configuration_group, configuration_subgroup, [name], [value], [visible])
...
-- =============================================
-- set process flag
CREATE PROCEDURE wf.sp_set_process_flag
...
CREATE PROCEDURE wf.sp_validate_schema AS
BEGIN
    SET NOCOUNT ON;
	...
	WHERE  ( table_schema = 'dbo' AND
				 table_name IN (SELECT [value] FROM STRING_SPLIT(@tables,',')));
    if(@returnValue = 20)
    BEGIN
    RETURN 1;
    END;
...
-- Get Record Counts
CREATE PROCEDURE wf.sp_get_current_record_counts AS
BEGIN
        SET NOCOUNT ON;
...
-- get record counts
CREATE PROCEDURE wf.sp_check_record_counts_changed AS
BEGIN
    SET NOCOUNT ON;
...
-- timeout jobs
CREATE PROCEDURE wf.sp_reset_job
AS
BEGIN
	SET NOCOUNT ON;
	UPDATE wf.[ssas_jobs] 
	...
	(SELECT [value] FROM wf.[configuration] WHERE [configuration_group] = 'SolutionTemplate' AND [configuration_subgroup]='SSAS' AND [name]='Timeout')
...
-- start job
CREATE PROCEDURE wf.sp_start_job
AS
BEGIN
	SET NOCOUNT ON;
	...
	EXEC wf.sp_reset_job
	
	INSERT wf.[ssas_jobs] (startTime, statusMessage)
	...
	if(@validateSchema = 1)
	BEGIN		
		EXECUTE @validateSchemaResult = wf.sp_validate_schema;
	...
	BEGIN
		...
		EXECUTE @checkRowCountsResult = wf.sp_check_record_counts_changed;
